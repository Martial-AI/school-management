...
class ActivityNotificationController extends Controller
{
    public function destroy(Request $request): JsonResponse
    {
        ...
        $activities = Activity::whereIn('id', $data['ids'])->get();
    ...
    public function markRead(Request $request): JsonResponse
    {
        ...
        $user = $request->user();
        $ids = $user->activity_notifications_read_ids ?? [];
        if (! in_array($data['id'], $ids, true)) $ids[] = $data['id'];
        $user->update(['activity_notifications_read_ids' => $ids]);
    ...
    public function feed(Request $request): JsonResponse
    {
        ...
        $activities = Activity::with(['causer', 'subject'])->where('created_at', '>', $after)->oldest()->get()->map(fn (Activity $activity) => [
            'id' => $activity->id,
        ...
        $readIds = $request->user()->activity_notifications_read_ids ?? [];
        $unreadCount = Activity::when($lastSeen, fn ($query) => $query->where('created_at', '>', $lastSeen))->whereNotIn('id', $readIds)->count();
